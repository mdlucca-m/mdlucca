...
**Variáveis.** BRUMS (seis subescalas + TMD), fadiga física e mental (0–10). Carga interna nas três sessões de HIIT: frequência cardíaca (pré/pós de cada série) e PSE. Adaptação física: velocidade de pico do teste de Carminatti (T-CAR) e salto vertical (CMJ).
...
### 3.5 Corroboração fisiológica e Fitness–Fadiga

A carga interna corrobora a leitura de acúmulo: entre a primeira e a terceira sessão de HIIT, a **FC de pico caiu (183 → 181 bpm; Friedman *p* < 0,001)** enquanto o esforço percebido se manteve alto e estável — o atleta entrega esforço máximo com um pico cardíaco cada vez menor, assinatura clássica de fadiga acumulada. No plano da adaptação, o mesociclo **funcionou**: a velocidade de pico do T-CAR subiu +0,93 km/h (dz = 1,32) e o CMJ +1,64 cm (dz = 0,85) — de modo que a fadiga captada na semana final é o **custo agudo de um bloco eficaz**, não sinal de estímulo insuficiente (dissociação Fitness–Fadiga). Por fim, a aptidão prévia **protege**: o atleta com maior velocidade de pico fadigou menos (T-CAR PV × fadiga física da semana *ρ* = −0,54; *p* = 0,005); como o estímulo foi prescrito a 104% da PV individual, o custo interno é equalizado e correr mais rápido em absoluto não implica fadigar mais.
