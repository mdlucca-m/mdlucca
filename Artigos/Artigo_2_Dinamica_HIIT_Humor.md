...
A prescrição relativa (4 séries × 4 min a 104% da PV do T-CAR; ≈ 2.929 m e velocidade média de 16,5 km/h por sessão) mantém a **carga externa idêntica** nas três sessões — um controle que faz da carga interna o revelador da fadiga. E ela revela: entre a primeira e a terceira sessão de HIIT (22 → 27/04), a **FC de pico caiu (183,8 → 180,8 bpm)** e o **TRIMP cardíaco declinou** (por supressão da FC sob fadiga), enquanto o esforço percebido **subiu** (session-RPE 196 → 208; PSE de 9–10 na série final) — o atleta entrega esforço máximo com um pico cardíaco cada vez menor. Trata-se da assinatura clássica de fadiga acumulada, aqui documentada **dentro da própria semana**, com um alerta prático: sob fadiga, o TRIMP baseado em FC **subestima** a carga, e o session-RPE é o marcador interno fiel (detalhes em `Analise_Carga_Interna_Externa_HIIT.md`). A aptidão prévia (covariável de base, avaliada em 15/04) **modera** essa resposta: o atleta com maior velocidade de pico do T-CAR fadigou menos ao longo da semana (T-CAR PV × fadiga física média *ρ* = −0,54; *p* = 0,005; × TMD *ρ* = −0,28; n.s.). Como o estímulo foi prescrito a 104% da PV individual, o custo interno é relativo e equalizado: correr mais rápido em absoluto não implica fadigar mais, e a vulnerabilidade à fadiga da semana associa-se a um traço de aptidão prévio. *Nota de escopo:* a magnitude da adaptação aeróbia/neuromuscular ao longo do mesociclo (ganhos de T-CAR e CMJ) situa-se fora da janela de 21–28/04 e, por isso, não é objeto deste artigo.

### 3.6 Outros instrumentos da semana: recuperação (TQR), sonolência (Epworth) e estresse (PSS)

Além do BRUMS, cada coleta da semana registrou a recuperação percebida (TQR, 6–20), a sonolência (Epworth adaptado, 0–18) e o estresse percebido (PSS-14). Analisados dentro da janela (Tabela 5), os três instrumentos separam-se nitidamente quanto à utilidade para o monitoramento do microciclo. A **TQR comporta-se como um membro do eixo energia–fadiga**: cai ao longo da semana (13,4 → 9,6; *p* < 0,001), responde agudamente ao treino (dz = −0,64; *p* = 0,001) e acopla-se fortemente ao humor intra-atleta (rmcorr com TMD = −0,65; fadiga = −0,64; vigor = +0,51) — é, portanto, um **marcador de recuperação genuinamente responsivo**. A **sonolência (Epworth)** capta o **acúmulo crônico** (8,1 → 11,2; *p* = 0,003) mas não responde à sessão isolada (dz = +0,18, n.s.). Já o **PSS não rastreia o microciclo**: sem efeito do dia (*p* = 0,20), sem resposta aguda (dz = +0,17, n.s.) e fracamente acoplado ao humor (rmcorr = +0,18) — coerente com sua natureza de instrumento retrospectivo ("no último mês"), dominado por variância de traço (ICC = 0,62). Em suma, **para o monitoramento diário do microciclo, TQR soma sinal (recuperação), Epworth adiciona a leitura de acúmulo, e o PSS é o instrumento errado** — mede o mês, não o dia.

**Tabela 5.** Comportamento dos demais instrumentos na semana (28 atletas, 456 obs; TQR n = 330).

| Instrumento | D1 → D7 | Efeito do dia (*p*) | Agudo (dz) | rmcorr × TMD | ICC traço | Leitura |
|---|---|---|---|---|---|---|
| **TQR** (recuperação 6–20) | 13,4 → 9,6 | < 0,001 | **−0,64** | −0,65 | 0,39 | responde agudo **e** crônico; eixo energia–fadiga |
| **Epworth** (sonolência 0–18) | 8,1 → 11,2 | 0,003 | +0,18 (n.s.) | +0,31 | 0,63 | **acúmulo crônico**; não agudo |
| **PSS** (estresse ~0–56) | 22,8 → 21,7 | 0,20 (n.s.) | +0,17 (n.s.) | +0,18 | 0,62 | **traço estável**; não rastreia o microciclo |
