...
**Escolher o instrumento certo para o horizonte certo.** O mesmo princípio traço/estado aplica-se aos demais questionários da semana. A recuperação percebida (TQR) tem variância de estado suficiente (ICC de traço = 0,39) e responde ao microciclo, somando-se ao eixo energia–fadiga; a sonolência (Epworth, ICC = 0,63) capta o acúmulo crônico; mas o **estresse percebido (PSS-14) é dominantemente traço (ICC = 0,62) e não rastreia o microciclo** — é um instrumento retrospectivo ("no último mês"), inadequado ao monitoramento diário (ver Artigo 2, §3.6). A lição psicométrica é geral: a fração de variância de estado de um instrumento — não sua popularidade — determina se ele serve para detectar mudança dentro de um microciclo.
