...
for r in range(MIC_F, MIC_L + 1):
    ...
    wsS.cell(r, 12, '=IF($K{0}=0,0,SUMIFS(\'Carga (PSE)\'!$G${1}:$G${2},\'Carga (PSE)\'!$B${1}:$B${2},$G{0})'
                    '/MAX(1,COUNTIF(Cadastro!$BP${3}:$BP${4},"Ativo"))/$K{0})'.format(r, CAR_F, CAR_L, CAD_F, CAD_L))
...
kpi(wsW, 30, 7, "CARGA REALIZADA — MÉDIA/ATLETA (UA)",
    ...
    "/MAX(1,COUNTIF(Cadastro!$BP${3}:$BP${4},\"Ativo\")),0)".format(0, CAR_F, CAR_L, CAD_F, CAD_L), NF_UA, GOLD, 3)
...
ATV = 'COUNTIF(Cadastro!$BP${}:$BP${},"Ativo")'.format(CAD_F, CAD_L)
kpi(wsD, 6, 1,  "ATLETAS ATIVOS", '={}'.format(ATV), '0', NAVY2, 2)
kpi(wsD, 6, 3,  "LESIONADOS / DM", '=COUNTIF(Cadastro!$BP${0}:$BP${1},"Lesionado")+COUNTIF(Cadastro!$BP${0}:$BP${1},"Departamento Médico")'.format(CAD_F, CAD_L), '0', RED_T, 2)
...
for i in range(CAD_L - CAD_F + 1):
    ...
    wsD.cell(r, 9, '=IF($A{0}="","",IF(Cadastro!$BP{1}="Lesionado","LESIONADO",IF($F{0}>1.5,"ALERTA — carga aguda alta",'
