...
def figura_prevalencia(destino: Path) -> Path:
    ...
    nosso = [PERFIS_T[p][1] for p in ordem]
    ...
    a1.barh([y - altura / 2 for y in ys], nosso, height=altura, color=E.TEAL,
            label="Esta amostra, dia 1 (escores T)", zorder=3)
    ...
    dif = [PERFIS_T[p][1] - NORMATIVO[p] for p in ordem]
    ...
    a2.set_xlabel("Diferença em relação à norma (p.p.)", fontsize=8.8,
                  color=E.TINTA)
    E.titulo(a2, "B. Diferença em relação à norma", tamanho=9.4)
