...
P('A caracterização do comportamento do humor em diferentes escalas temporais — no conjunto da semana, na variação da linha de base ao último dia e na resposta aguda dentro de cada dia — e, simultaneamente, nos níveis coletivo e individual, fornece um panorama interpretável para calibrar a carga e individualizar a recuperação. Este estudo tem por objetivo avaliar e classificar o perfil de humor de atletas de handebol durante a última semana da pré-temporada competitiva e, em especial, modelar a resposta de fadiga por meio de ajustes alométrico e logístico — normalizando as variáveis pela massa corporal e pela aptidão aeróbia (pico de velocidade) e estabelecendo o limiar de aptidão associado à alta fadiga —, descrevendo ainda o comportamento geral do grupo por variável (incluindo a PTH), a variação da linha de base (Dia 1, pré) ao último dia (Dia 7, pós), a resposta pré→pós e a sensibilidade relativa das variáveis.')
P('A escolha por modelos alométricos e logísticos fundamenta-se em evidência consolidada nas modalidades coletivas. O escalonamento alométrico — que expressa a variável como potência da massa corporal (Y = a·massaᵇ), e não como razão simples por quilograma — é o procedimento recomendado para comparar a aptidão de atletas de portes distintos: em jogadores de futebol, o consumo máximo de oxigênio escala com a massa elevada a ≈ 0,72, e não a 1,0 (CHAMARI et al., 2005), e uma metanálise com mais de 6.500 indivíduos confirmou que o expoente de escalonamento difere sistematicamente da unidade (LOLLI et al., 2017), de modo que a razão por quilograma enviesa as comparações a favor dos atletas mais leves. Já a regressão logística associada à curva ROC tornou-se padrão para converter indicadores contínuos de monitoramento em limiares de decisão prática (a probabilidade de um desfecho binário, como alto risco ou alta fadiga): em futebol profissional, razões de carga externa foram modeladas por regressão logística e ROC para prever, semana a semana, o risco de lesão (FOUSEKIS et al., 2025). O presente estudo transpõe esses dois modelos, já validados em esportes coletivos para carga e desempenho, ao monitoramento do humor e da fadiga.')
...
rows=[[ANORM[v]['lab'],c2(f"{ANORM[v]['rho_mass']:+.2f}"),c2(f"{ANORM[v]['rho_after_mass']:+.2f}"),c2(f"{ANORM[v]['rho_pv']:+.2f}"),c2(f"{ANORM[v]['rho_after_pv']:+.2f}")] for v in order]
...
sec('REFERÊNCIAS','')
for r in [
 'CHAMARI, K. et al. Appropriate interpretation of aerobic capacity: allometric scaling in adult and young soccer players. British Journal of Sports Medicine, v. 39, n. 2, p. 97-101, 2005.',
 'FOUSEKIS, A. et al. Week-by-week predictive value of external load ratios on injury risk in professional soccer: a logistic regression and ROC curve analysis approach. Medicina, v. 61, n. 11, art. 1954, 2025.',
 'LOLLI, L.; BATTERHAM, A. M.; WESTON, K. L.; ATKINSON, G. Size exponents for scaling maximal oxygen uptake in over 6500 humans: a systematic review and meta-analysis. Sports Medicine, v. 47, n. 7, p. 1405-1419, 2017.',
