...
MS=json.load(open('model_stats.json')); S4=json.load(open('brums_stats4.json')); CRS=json.load(open('cross.json')); STAT2=json.load(open('brums_stats3.json'))
...
P('Objetivos específicos: (i) caracterizar a amostra quanto aos aspectos sociodemográficos e antropométricos; (ii) '
 ...
 'acompanhar a evolução dos perfis de humor; (vi) comparar o comportamento das variáveis dia a dia e do primeiro ao '
 'último dia, com estimativa do tamanho do efeito; (vii) identificar as dimensões mais sensíveis e a estrutura de '
 'correlação entre elas; e (viii) analisar a aptidão aeróbia intermitente (pico de velocidade do T-CAR) como parâmetro '
 'fisiológico, estabelecendo um limiar de pico de velocidade associado aos dias de maior fadiga.')
...
P('Os dados foram analisados por estatística descritiva (média, desvio-padrão, mediana e amplitude). A normalidade das '
 ...
 'de frequências. Além do valor de p, estimou-se o tamanho do efeito das comparações pré→pós e Dia 1→Dia 7 pelo d de '
 'Cohen (classificado como trivial < 0,2; pequeno < 0,5; médio < 0,8; grande ≥ 0,8). As diferenças entre os sete dias '
 'foram testadas pelo teste de Friedman, e a associação entre as dimensões, pela correlação de Spearman (ρ). A relação '
 'entre o pico de velocidade do T-CAR e a fadiga foi analisada por regressão, e um limiar de pico de velocidade foi '
 'estabelecido para discriminar os dias de maior fadiga. Adotou-se nível de significância de 5% (p < 0,05).')
...
table('Diferenças entre pré e pós-treino (média agregada da semana; Wilcoxon e tamanho de efeito).',
    ['Dimensão','Pré (M)','Pós (M)','Variação (%)','p','d de Cohen','Magnitude'],[ppr(k,l) for k,l in ORD],
    note='d de Cohen: trivial < 0,2; pequeno < 0,5; médio < 0,8; grande ≥ 0,8.',fs=9)
H('4.5 Comportamento das variáveis dia a dia',12,before=6)
...
FR=S4['friedman']
def ddrow(k,lab):
    dm=CRS['dec'][k]['dm']; fr=FR.get(k)
    cells=[lab]+[c2('%.1f'%dm[str(d)]) for d in range(1,8)]
    cells.append(pstr(fr['p']) if fr else '—'); return cells
P('A Tabela 6 apresenta as médias diárias de cada dimensão e o teste de Friedman, que avalia se há diferença entre os '
 'sete dias. As diferenças foram significativas para o vigor (p = %s), a fadiga (p = %s), a tensão (p = %s) e a confusão '
 '(p = %s), reforçando que o eixo energia–fadiga é o que mais se modifica ao longo da semana.'%(
   pstr(FR['Vigor']['p']),pstr(FR['Fadiga']['p']),pstr(FR['Tensao']['p']),pstr(FR['Confusao']['p'])))
table('Médias diárias das dimensões do BRUMS e teste de Friedman (diferença entre os sete dias).',
    ['Dimensão','D1','D2','D3','D4','D5','D6','D7','p (Friedman)'],
    [ddrow(k,l) for k,l in ORD],fs=8.5)
H('4.6 Comparação do primeiro ao último dia (Dia 1 → Dia 7)',12,before=6)
d17=R['d1d7']
P('A comparação entre o primeiro e o último dia (Tabela 7) confirma o padrão, agora com o tamanho do efeito: o vigor '
 'caiu %s%% (d = %s; efeito grande) e a fadiga aumentou %s%% (d = %s; efeito médio), enquanto as dimensões negativas '
 'apresentaram efeitos pequenos ou triviais.'%(
   c2('%.0f'%abs(d17['Vigor']['pct'])),c2('%.2f'%d17['Vigor']['dz']),c2('%.0f'%d17['Fadiga']['pct']),c2('%.2f'%d17['Fadiga']['dz'])))
def d17row(k,lab):
    v=d17[k]; return [lab,c2('%.2f'%v['d1']),c2('%.2f'%v['d7']),c2('%+.2f'%v['delta']),c2('%+.0f'%v['pct'])+'%',pstr(v['p']),c2('%+.2f'%v['dz']),v['mag']]
table('Comparação do Dia 1 ao Dia 7, por dimensão (Wilcoxon e tamanho de efeito).',
    ['Dimensão','Dia 1 (M)','Dia 7 (M)','Δ','Variação (%)','p','d de Cohen','Magnitude'],
    [d17row(k,l) for k,l in ORD],fs=8.5)
H('4.7 Dimensões mais sensíveis e correlação entre as dimensões',12,before=6)
SENS=S4['sens']
snames={'Vigor':'Vigor','Fadiga':'Fadiga','TMD':'PTH','Tensao':'Tensão','Depressao':'Depressão','Raiva':'Raiva','Confusao':'Confusão'}
sord=sorted(snames,key=lambda k:-abs(SENS[k]['dz']))
P('Ordenando as variáveis pela magnitude do efeito ao longo da semana (Tabela 8), o vigor (d = %s) e a fadiga (d = %s) '
 'foram as mais sensíveis, seguidos da tensão e da confusão; a depressão e a raiva foram as menos sensíveis. A correlação '
 'entre as dimensões (Tabela 9) mostrou que as dimensões negativas se associam entre si e que a fadiga se relaciona com a '
 'depressão e a raiva, ao passo que o vigor se mantém relativamente independente.'%(
   c2('%+.2f'%SENS['Vigor']['dz']),c2('%+.2f'%SENS['Fadiga']['dz'])))
table('Dimensões ordenadas pela magnitude do efeito (Dia 1 → Dia 7).',
    ['Ordem','Dimensão','d de Cohen','Magnitude'],
    [[i+1,snames[k],c2('%+.2f'%SENS[k]['dz']),('grande' if abs(SENS[k]['dz'])>=0.8 else 'médio' if abs(SENS[k]['dz'])>=0.5 else 'pequeno' if abs(SENS[k]['dz'])>=0.2 else 'trivial')] for i,k in enumerate(sord)],fs=9)
PA=STAT2['pairs']
sig=[pr_ for pr_ in PA if pr_['p']<0.05]
table('Correlação de Spearman entre as dimensões do BRUMS com associação significativa (p < 0,05).',
    ['Par de dimensões','ρ','p'],[['%s × %s'%(pr_['a'],pr_['b']),c2('%+.2f'%pr_['rho']),pstr(pr_['p'])] for pr_ in sig],fs=9)
H('4.8 Classificação dos perfis de humor',12,before=6)
P('A classificação nos seis perfis de humor evidenciou a migração ao longo da semana (Figura 2; Tabela 10). No primeiro '
...
# ===== 4.7 T-CAR + LIMIAR =====
H('4.9 Aptidão intermitente (T-CAR) e limiar de pico de velocidade',12,before=6)
P('O desempenho no T-CAR está descrito na Tabela 11. O pico de velocidade inicial (T-CAR1) foi de %s ± %s km/h, servindo '
...
OUTP='/home/user/mdlucca/Artigos/Relatorio_Intermediario_Perfil_Humor.docx'
