...
VARS=[('FadFisica','Fadiga física (0–10)','#f03e3e'),('FadMental','Fadiga mental (0–10)','#ffd43b')]
...
raw=hum[keys].dropna()
r_obs=raw['FadFisica'].corr(raw['FadMental'])
r_dis=r_obs/np.sqrt(grp['FadFisica']['rel']*grp['FadMental']['rel'])
gm=hum.groupby('dia')[keys].mean(); r_den=gm['FadFisica'].corr(gm['FadMental'])
rho_obs=stats.spearmanr(raw['FadFisica'],raw['FadMental']).correlation
...
rm=rmcorr(hum,'FadFisica','FadMental')
...
print('corr física×mental: obs r=%.2f rho=%.2f | disatt=%.2f | group=%.2f | rmcorr=%.2f'%(r_obs,rho_obs,r_dis,r_den,rm))
...
fig.update_xaxes(dtick=1,title='Dia'); fig.update_yaxes(range=[0,10]); fig.write_image(f'{FIG}/pf1_trajetoria.png',width=3840,height=2160)

# PF2 both trajectories overlaid (dissociation)
fig=go.Figure()
for v,lab,c in VARS:
    d=hum.dropna(subset=[v]); lo=lowess(d[v],d['dia'],frac=0.6,return_sorted=True); dm=d.groupby('dia')[v].mean()
    fig.add_trace(go.Scatter(x=dm.index,y=dm.values,mode='markers',marker=dict(color=c,size=8,opacity=.5),showlegend=False))
    fig.add_trace(go.Scatter(x=lo[:,0],y=lo[:,1],mode='lines',name=lab,line=dict(color=c,width=4)))
fig.update_layout(**T,height=520,title='Fig 2 — Dissociação: a fadiga física sobe; a mental fica plana',xaxis=dict(dtick=1,title='Dia'),yaxis=dict(title='Escore (0–10)',range=[0,10]),legend=dict(orientation='h',y=-0.18))
fig.write_image(f'{FIG}/pf2_dissociacao.png',width=3840,height=2160)
...
fig.write_image(f'{FIG}/pf3_amp_box.png',width=3840,height=2160)
...
fig.update_xaxes(dtick=1,title='Dia'); fig.update_yaxes(range=[0,10]); fig.write_image(f'{FIG}/pf4_prepos.png',width=3840,height=2160)
...
xj=raw['FadFisica']+rng.normal(0,0.08,len(raw)); yj=raw['FadMental']+rng.normal(0,0.08,len(raw))
fig.add_trace(go.Scatter(x=xj,y=yj,mode='markers',marker=dict(color='#868e96',size=5,opacity=.35),showlegend=False),1,1)
b=np.polyfit(raw['FadFisica'],raw['FadMental'],1); xs=np.array([0,10]); fig.add_trace(go.Scatter(x=xs,y=np.polyval(b,xs),mode='lines',line=dict(color='#f03e3e',width=3),showlegend=False),1,1)
fig.add_trace(go.Scatter(x=gm['FadFisica'],y=gm['FadMental'],mode='markers+text',text=[f'D{d}' for d in gm.index],textposition='top center',marker=dict(color='#ffd43b',size=12),showlegend=False),1,2)
b2=np.polyfit(gm['FadFisica'],gm['FadMental'],1); xs2=np.linspace(gm['FadFisica'].min(),gm['FadFisica'].max(),2); fig.add_trace(go.Scatter(x=xs2,y=np.polyval(b2,xs2),mode='lines',line=dict(color='#ffd43b',width=3),showlegend=False),1,2)
fig.update_layout(**T,height=520,title='Fig 5 — Relação fadiga física × mental: bruta (com ruído) vs nível-grupo (denoised)')
fig.update_xaxes(title='Fadiga física (0–10)'); fig.update_yaxes(title='Fadiga mental (0–10)')
fig.write_image(f'{FIG}/pf5_scatter.png',width=3840,height=2160)
...
fig.update_xaxes(title='k coletas',dtick=1); fig.write_image(f'{FIG}/pf6_mdc.png',width=3840,height=2160)
print('figs OK')
...
json.dump(G,open(f'{SC}/pair_fatigue.json','w'),indent=1)
print('saved pair_fatigue.json')
