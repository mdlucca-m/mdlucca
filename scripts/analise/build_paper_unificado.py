...
PHY=json.load(open('physio2.json'))
def pr3(p): return '< 0,001' if p<0.001 else ('= '+('%.3f'%p).replace('.',','))
def rr(x,d=2): return ('%+.*f'%(d,x)).replace('.',',')
...
r=p.add_run('DINÂMICA DO HUMOR E APTIDÃO INTERMITENTE EM UM MICROCICLO PRÉ-COMPETITIVO DE HANDEBOL DE ELITE: BRUMS, PICO DE VELOCIDADE NO T-CAR E CARGA DO HIIT')
r.bold=True; r.font.size=Pt(13); p.paragraph_format.space_after=Pt(4)
p=doc.add_paragraph(); p.alignment=WD_ALIGN_PARAGRAPH.CENTER
r=p.add_run('(versão unificada: humor, aptidão e carga, com todas as análises no corpo do texto)'); r.italic=True; r.font.size=Pt(11); p.paragraph_format.space_after=Pt(10)
...
RUN([('Objetivo: ',True),(f'descrever e caracterizar a dinâmica do humor de atletas de handebol de elite ao longo de um '
 ...
 f'dia) para a barbatana de tubarão ({bar_d7}% no último dia), com aumento da chance desse perfil a cada dia (OR = {or_bar}). '
 f'A deterioração concentrou-se nos dias com HIIT (vigor com dz = {rr(PHY["hiit_vs_noh"]["Vigor"]["dz"])}; PTH com '
 f'dz = {rr(PHY["hiit_vs_noh"]["TMD"]["dz"])}), e a aptidão aeróbia intermitente (pico de velocidade no T-car) associou-se '
 f'a mais vigor (ρ = {rr(PHY["pv_week"]["Vigor"]["rho"])}) e menos fadiga física (ρ = {rr(PHY["pv_week"]["FadFisica"]["rho"])}) '
 f'ao longo da semana, com um limiar em torno de 14,9 km/h a discriminar os atletas de maior fadiga (AUC = {num(PHY["roc"]["FadFisica"]["auc"],2)}). ',False),
 ('Conclusão: ',True),(f'o humor migrou da prontidão para a fadiga funcional, conduzido pelos dias de HIIT e modulado '
 f'pela aptidão, em um padrão compatível com sobre-esforço funcional, o que recomenda centrar o monitoramento no par '
 f'vigor–fadiga, integrado à aptidão e à carga.',False)],after=6)
P('Palavras-chave: humor; BRUMS; handebol; pico de velocidade; teste de Carminatti; HIIT; aptidão aeróbia; fadiga; monitoramento do atleta.',size=11,after=8,ind=False)
...
P('g) Integrar o humor aos dados fisiológicos e de carga, com a análise da relação entre a aptidão aeróbia intermitente '
 '(pico de velocidade no T-car), os dias com e sem HIIT e as dimensões do humor, e derivar limiares de aptidão para '
 'discriminar os atletas de maior fadiga.',ind=False)
...
P('A forma temporal das trajetórias foi analisada por três abordagens complementares. Primeiro, as séries de vigor, '
 ...
 'significância de 5%.')
P('Além do humor, o estudo reuniu medidas fisiológicas e de contexto. A aptidão aeróbia intermitente foi avaliada pelo '
 'pico de velocidade no teste de Carminatti (T-car), aplicado na linha de base, definido como a velocidade do último '
 'estágio completado no protocolo incremental intermitente vaivém. A fadiga física e a fadiga mental foram registradas '
 'por itens específicos de autorrelato, em complemento à fadiga do BRUMS. A carga interna das sessões de HIIT do '
 'microciclo, realizadas nos dias 2, 4 e 7, foi monitorada pela frequência cardíaca de esforço e pela percepção '
 'subjetiva de esforço ao fim de cada série. A sonolência diurna foi medida pela Escala de Sonolência de Epworth e o '
 'estresse percebido pela escala de estresse percebido (PSS).')
P('A integração fisiológica seguiu quatro passos. As relações entre o pico de velocidade e o humor, de linha de base e '
 'semanal, foram estimadas por correlações de Spearman em todas as dimensões, inclusive a depressão. Os atletas foram '
 'divididos em tercis de aptidão, comparados por Kruskal-Wallis. As diferenças de humor entre os dias com e sem HIIT '
 'foram testadas por Wilcoxon pareado, com o tamanho de efeito. A capacidade do pico de velocidade de discriminar os '
 'atletas de maior sobrecarga foi avaliada por curvas ROC, com o limiar pelo índice de Youden, e a relação entre a '
 'aptidão e a fadiga foi ainda examinada sob ajuste alométrico pela massa corporal.')
...
P('Os resultados são apresentados em três níveis de profundidade, em ordem lógica. O primeiro nível, descritivo, resume '
 ...
 'recuperação ao longo das treze coletas. O quarto nível, de integração, relaciona o humor à aptidão aeróbia '
 'intermitente (pico de velocidade no T-car), aos dias com e sem HIIT, à carga interna, ao sono e ao estresse, e deriva '
 'limiares de discriminação. Cada análise é descrita e, em seguida, interpretada quanto ao seu significado para o '
...
PVd=PHY['tcar']['PVpre']; PVpos=PHY['tcar']['PVpos']; dPV=PHY['tcar']['dPV']
H('5.10 Aptidão intermitente (T-car) e humor ao longo da semana',12,before=6)
P(f'A aptidão aeróbia intermitente foi indexada pelo pico de velocidade no teste de Carminatti (T-car), medido na linha '
 f'de base. O pico de velocidade foi de {num(PVd["m"],1)} ± {num(PVd["sd"],1)} km/h ({PVd["n"]} atletas), com amplitude '
 f'de {num(PVd["mn"],1)} a {num(PVd["mx"],1)} km/h. A relação entre a aptidão e o humor médio da semana concentrou-se, '
 f'mais uma vez, no eixo energia–fadiga (Figura {_FN[0]+1}; Tabela {_TN[0]+1}): quanto maior o pico de velocidade, maior '
 f'o vigor (ρ = {rr(PHY["pv_week"]["Vigor"]["rho"])}; p {pr3(PHY["pv_week"]["Vigor"]["p"])}) e menor a fadiga física '
 f'(ρ = {rr(PHY["pv_week"]["FadFisica"]["rho"])}; p {pr3(PHY["pv_week"]["FadFisica"]["p"])}), com tendência semelhante na '
 f'PTH e na fadiga. A depressão não se associou de forma significativa à aptidão (ρ = {rr(PHY["pv_week"]["Depressao"]["rho"])}; '
 f'p {pr3(PHY["pv_week"]["Depressao"]["p"])}), resultado coerente com o forte efeito de piso da dimensão.')
figure(f'{FG}/pv_mood.png','Relação entre o pico de velocidade no T-car e o vigor e a fadiga física médios da semana, com a reta de regressão e o coeficiente de Spearman.',w=15.0)
PVW=[('Vigor','Vigor'),('Fadiga','Fadiga'),('FadFisica','Fadiga física'),('TMD','PTH'),('Depressao','Depressão'),('Tensao','Tensão'),('Raiva','Raiva'),('Confusao','Confusão')]
def pvrow(k,lab,src):
    v=PHY[src][k]; return [lab,rr(v['rho']),pvt(v['p']),str(v['n'])]
table('Correlação de Spearman entre o pico de velocidade no T-car e o humor semanal e de linha de base (todas as dimensões).',
 ['Dimensão','ρ (semanal)','p (semanal)','ρ (linha de base)','p (linha de base)'],
 [[lab,rr(PHY['pv_week'][k]['rho']),pvt(PHY['pv_week'][k]['p']),rr(PHY['pv_base'][k]['rho']),pvt(PHY['pv_base'][k]['p'])] for k,lab in PVW],
 note='ρ = coeficiente de Spearman; n = %d atletas com T-car. A depressão é apresentada apesar do efeito de piso.'%PVd['n'],fs=8.5)
tc=PHY['terc_pv']
P(f'Para tornar essa relação mais legível, os atletas foram divididos em tercis de aptidão (pico de velocidade médio de '
 f'{num(tc["Baixa"],1)}, {num(tc["Média"],1)} e {num(tc["Alta"],1)} km/h). O padrão foi dose-dependente (Figura {_FN[0]+1}): '
 f'o tercil mais apto apresentou o maior vigor semanal ({num(PHY["tercis"]["Vigor"]["Alta"],1)} contra '
 f'{num(PHY["tercis"]["Vigor"]["Baixa"],1)} no menos apto; Kruskal-Wallis p {pr3(PHY["tercis"]["Vigor"]["p"])}) e a menor '
 f'fadiga física ({num(PHY["tercis"]["FadFisica"]["Alta"],1)} contra {num(PHY["tercis"]["FadFisica"]["Baixa"],1)}; '
 f'p {pr3(PHY["tercis"]["FadFisica"]["p"])}). A depressão também diferiu entre os tercis (p {pr3(PHY["tercis"]["Depressao"]["p"])}), '
 f'com escores mais altos no grupo menos apto, o que sugere uma associação da aptidão com o afeto negativo apesar do piso.')
figure(f'{FG}/pv_tercis.png','Humor semanal médio por tercil de aptidão (pico de velocidade no T-car). Valores de p do teste de Kruskal-Wallis.',w=15.0)
al=PHY['allometry']; ae=PHY['allo_effect']
P(f'A relação entre a aptidão e a fadiga resistiu a um ajuste alométrico pela massa corporal. O pico de velocidade '
 f'escalonou com a massa por um expoente de {num(al["expoente"],2)}, e, após a normalização alométrica, a correlação '
 f'entre a aptidão e a fadiga física manteve-se ou fortaleceu-se (de ρ = {rr(ae["bruto_rho"])} para '
 f'ρ = {rr(ae["allo_rho"])}), o que indica que a associação não é um mero artefato do tamanho corporal.')

H('5.11 Dias com vs. sem HIIT',12,before=6)
hv=PHY['hiit_vs_noh']; ld=PHY['hiit_load']
P(f'O microciclo alternou dias com treinamento intervalado de alta intensidade (HIIT), nos dias 2, 4 e 7, e dias sem '
 f'HIIT, nos dias 1, 3, 5 e 6. A comparação do humor entre esses dias, atleta a atleta (Figura {_FN[0]+1}; '
 f'Tabela {_TN[0]+1}), mostrou que a deterioração do estado se concentra nos dias de HIIT: o vigor foi menor '
 f'(dz = {rr(hv["Vigor"]["dz"])}; p {pr3(hv["Vigor"]["p"])}) e a fadiga, a fadiga física e a PTH foram maiores nesses '
 f'dias (PTH com dz = {rr(hv["TMD"]["dz"])}; p {pr3(hv["TMD"]["p"])}), todos com efeito de moderado a grande. A depressão '
 f'subiu nos dias de HIIT, mas sem significância (dz = {rr(hv["Depressao"]["dz"])}; p {pr3(hv["Depressao"]["p"])}), de '
 f'novo limitada pelo piso.')
figure(f'{FG}/hiit_contrast.png','Humor médio nos dias com e sem HIIT, por dimensão (Wilcoxon pareado; * p < 0,05). Azul: dias sem HIIT; laranja: dias com HIIT.',w=15.0)
HRO=[('Vigor','Vigor'),('Fadiga','Fadiga'),('FadFisica','Fadiga física'),('TMD','PTH'),('Depressao','Depressão'),('Raiva','Raiva'),('Tensao','Tensão'),('Confusao','Confusão')]
table('Humor nos dias com e sem HIIT: médias, tamanho de efeito e teste de Wilcoxon pareado.',
 ['Dimensão','Sem HIIT','Com HIIT','dz','p','Magnitude'],
 [[lab,num(hv[k]['noh'],2),num(hv[k]['hiit'],2),rr(hv[k]['dz']),pvt(hv[k]['p']),
   ('trivial' if abs(hv[k]['dz'])<0.2 else 'pequeno' if abs(hv[k]['dz'])<0.5 else 'médio' if abs(hv[k]['dz'])<0.8 else 'grande')] for k,lab in HRO],
 note='dz = tamanho de efeito intraindividual; n = %d atletas. PTH: perturbação total do humor.'%hv['Vigor']['n'],fs=8.5)
P(f'A carga interna das próprias sessões de HIIT confirma a sua intensidade (Figura {_FN[0]+1}). A frequência cardíaca '
 f'de esforço situou-se em torno de {num(ld["mean_fc"]["m"],0)} bpm, próxima de {num(ld["pctFCmax"]["m"],0)}% da '
 f'frequência cardíaca máxima, e a percepção de esforço ao fim das séries alcançou {num(ld["final_pse"]["m"],1)} em dez, '
 f'com a sessão do Dia 7 a somar a maior percepção de esforço à fadiga terminal da semana.')
figure(f'{FG}/hiit_load.png','Carga interna das sessões de HIIT do microciclo: frequência cardíaca de esforço (barras) e percepção subjetiva de esforço ao fim das séries (linha).',w=13.0)

H('5.12 Limiares de aptidão para discriminar a fadiga (ROC)',12,before=6)
roc=PHY['roc']
P(f'A capacidade do pico de velocidade de discriminar os atletas de maior sobrecarga na semana foi avaliada por curvas '
 f'ROC (Figura {_FN[0]+1}; Tabela {_TN[0]+1}). O pico de velocidade separou de forma satisfatória os atletas de maior '
 f'fadiga física (AUC = {num(roc["FadFisica"]["auc"],2)}) e de menor vigor (AUC = {num(roc["Vigor"]["auc"],2)}), com um '
 f'limiar em torno de {num(roc["FadFisica"]["cut"],1)} km/h que combinou sensibilidade de {num(100*roc["FadFisica"]["sens"],0)}% '
 f'e especificidade de {num(100*roc["FadFisica"]["spec"],0)}%. Atletas abaixo desse limiar concentraram os maiores '
 f'escores de fadiga ao longo da semana, o que sugere um ponto de corte prático para sinalizar quem exige maior atenção '
 f'no monitoramento.')
figure(f'{FG}/roc_pv.png','Curvas ROC do pico de velocidade no T-car para discriminar alta fadiga física, baixa vigor e alta PTH na semana, com a área sob a curva (AUC).',w=11.5)
table('Discriminação da sobrecarga semanal pelo pico de velocidade no T-car: área sob a curva ROC e limiar (índice de Youden).',
 ['Alvo (metade mais sobrecarregada)','AUC','Limiar PV (km/h)','Sensibilidade','Especificidade'],
 [[LAB,num(roc[k]['auc'],2),num(roc[k]['cut'],1),num(100*roc[k]['sens'],0)+'%',num(100*roc[k]['spec'],0)+'%'] for k,LAB in [('FadFisica','Alta fadiga física'),('Fadiga','Alta fadiga (BRUMS)'),('TMD','Alta PTH'),('Vigor','Baixo vigor')]],
 note='Limiar pelo índice de Youden. AUC 0,7–0,8 = discriminação aceitável; > 0,8 = boa.',fs=8.5)

H('5.13 Sono, estresse percebido e humor',12,before=6)
ep=PHY['epworth']; ps=PHY['pss']; ss=PHY['sleep_stress']
P(f'Por fim, dois moderadores do estado foram considerados: a sonolência diurna (Escala de Epworth, {num(ep["m"],1)} ± '
 f'{num(ep["sd"],1)}) e o estresse percebido (PSS, {num(ps["m"],1)} ± {num(ps["sd"],1)}). A sonolência associou-se à '
 f'fadiga da semana (ρ = {rr(ss["Epworth"]["Fadiga"]["rho"])}; p {pr3(ss["Epworth"]["Fadiga"]["p"])}) e à PTH '
 f'(ρ = {rr(ss["Epworth"]["TMD"]["rho"])}; p {pr3(ss["Epworth"]["TMD"]["p"])}), o que indica que o atleta mais sonolento '
 f'tende a relatar mais fadiga. O estresse percebido não se correlacionou de forma significativa com o humor semanal '
 f'(todos p > 0,05), o que sugere que, neste microciclo, a variação do humor respondeu mais à carga física e ao sono do '
 f'que ao estresse psicossocial geral.')
...
H('6.5 Aptidão, carga do HIIT e a resposta do humor',12,before=6)
P('A integração com os dados fisiológicos deu contexto de carga à dinâmica do humor. A alternância entre dias com e sem '
 'HIIT explicou boa parte da deterioração observada: foi nos dias de treinamento intervalado, com frequência cardíaca '
 'elevada e percepção de esforço próxima do máximo, que o vigor caiu e a fadiga e a PTH subiram com efeito de moderado '
 'a grande. Esse acoplamento entre a carga aguda e a resposta do humor reforça a leitura do eixo energia–fadiga como um '
 'marcador sensível do custo do treino, complementar aos indicadores de carga interna (SAW; MAIN; GASTIN, 2016; '
 'HELWIG et al., 2023).')
P('A aptidão aeróbia intermitente, indexada pelo pico de velocidade no T-car, comportou-se como um moderador do estado. '
 'Os atletas mais aptos mantiveram mais vigor e menos fadiga física ao longo da semana, uma associação que resistiu ao '
 'ajuste alométrico pela massa corporal e que se traduziu em limiares úteis: um pico de velocidade em torno de 14,9 km/h '
 'discriminou de forma aceitável os atletas de maior fadiga. Esse achado sugere que a aptidão funciona como uma reserva '
 'que atenua o impacto psicológico da carga, em linha com o papel protetor do condicionamento aeróbio descrito em '
 'esportes intermitentes (KARCHER; BUCHHEIT, 2014; GARCÍA-SÁNCHEZ et al., 2023).')
P('A depressão merece uma leitura cautelosa. Embora as correlações com a aptidão e com a carga tenham sido fracas e não '
 'significativas, o forte efeito de piso, com a maioria dos escores em zero, limita a variância disponível e reduz a '
 'sensibilidade de qualquer teste sobre essa dimensão. A diferença entre os tercis de aptidão, com escores um pouco mais '
 'altos no grupo menos apto, e a leve elevação nos dias de HIIT indicam uma tendência coerente, ainda que dominada pelo '
 'piso, o que recomenda interpretar a depressão neste contexto por prevalência e por perfis, e não por escore contínuo '
 '(ROHLFS et al., 2023; TERRY et al., 2022).')
P('Por fim, o sono ajudou a compor o quadro. A associação entre a sonolência diurna e a fadiga da semana indica que o '
 'estado de humor do atleta responde não só à carga de treino, mas também à qualidade da recuperação, o que amplia a '
 'recomendação de um monitoramento multidomínio que combine humor, aptidão, carga e sono (KELLMANN et al., 2018).')
H('6.6 Limitações',12,before=6)
...
OUTP='/home/user/mdlucca/Artigos/Paper_Unificado_Humor_TCAR_HIIT.docx'
