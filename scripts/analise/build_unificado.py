...
TCD=json.load(open('tcar_desc.json'))
...
P('A aptidão aeróbia intermitente foi avaliada pelo Teste de Carminatti (T-CAR), teste de campo progressivo e '
 ...
 'al., 2016). O T-CAR foi aplicado em 15 de abril de 2024, quatro dias de treino antes do início do microciclo monitorado '
 '(21–27 de abril de 2024), de modo a funcionar como parâmetro fisiológico (covariável) de linha de base das análises. O '
 'pico de velocidade inicial (T-CAR1; PV = %s ± %s km/h) foi reaplicado ao final da pré-temporada (T-CAR2; PV = %s ± %s '
 'km/h), caracterizando a aptidão inicial de cada atleta e sua variação (ΔPV = %s ± %s km/h). A caracterização completa do '
 'desempenho no teste consta na Tabela 3. O PV inicial (T-CAR1) foi utilizado como parâmetro fisiológico de referência '
 'para normalizar e interpretar interindividualmente a resposta de humor–fadiga ao microciclo.'%(
...
def tcrow(o):
    return [o['lab'],o['n'],c2('%.1f'%o['m']),c2('%.1f'%o['sd']),'%s–%s'%(c2('%.1f'%o['mn']),c2('%.1f'%o['mx'])),c2('%.0f'%o['cv'])]
t_tcar=table('Estatística descritiva do desempenho no Teste de Carminatti (T-CAR), covariável fisiológica do estudo.',
    ['Parâmetro','n','Média','DP','Mín–Máx','CV (%)'],[tcrow(o) for o in TCD],
    note='PV = pico de velocidade; FC = frequência cardíaca; TRIMP = training impulse; CV = coeficiente de variação. T-CAR1 (linha de base) e T-CAR2 (final da pré-temporada).',fs=8.5)
...
P('O teste de Shapiro-Wilk rejeitou a normalidade em todas as dimensões (p < 0,001; Tabela 5), com as negativas '
 ...
 'dimensões constam na Tabela 4. O vigor (escore T de %s a %s) e a fadiga concentram a variabilidade informativa, '
 'enquanto as dimensões negativas apresentam medianas baixas e coeficientes de variação inflados pelo efeito de piso '
 '(Tabela 6). As dimensões negativas intercorrelacionam-se entre si e a fadiga associa-se à depressão e à raiva, ao passo '
...
OUTP='/home/user/mdlucca/Artigos/Artigo_Unificado_Perfil_Humor_Handebol.docx'
