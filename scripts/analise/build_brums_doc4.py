...
P('A normalidade das distribuições foi verificada pelo teste de Shapiro-Wilk e pela inspeção de histogramas e diagramas '
 'de caixa. Como todas as subescalas violaram a normalidade (Shapiro-Wilk p < 0,001) e as dimensões negativas '
 'apresentaram forte assimetria positiva e efeito de piso, adotaram-se procedimentos não paramétricos. Reportou-se '
 'estatística descritiva (média, mediana, desvio-padrão, IC95%, coeficiente de variação e amplitude), no geral, por dia e '
 'nos momentos pré e pós. A consistência das medidas repetidas foi estimada pelo coeficiente de correlação intraclasse '
 '(ICC, modelo de duas vias, efeitos aleatórios; ICC(2,1) para uma medida e ICC(2,k) para a média da semana), '
 'interpretado como pobre (< 0,50), moderado (0,50–0,75), bom (0,75–0,90) ou excelente (> 0,90). As comparações pré→pós e '
 'Dia 1→Dia 7 empregaram o teste de Wilcoxon pareado, com variação absoluta e percentual, tamanho de efeito pareado '
 '(dz de Cohen) com IC95% e classificação de magnitude (trivial < 0,2; pequeno < 0,5; médio < 0,8; grande ≥ 0,8). A '
 'associação entre subescalas foi examinada por correlação de Spearman (ρ), com valores de p, no nível entre atletas e '
 'nos dias de maior vigor e de maior fadiga. No nível do sujeito, calculou-se o Índice de Mudança Confiável '
 '(RCI = (D7 − D1)/EP_dif; EP_dif = DP_D1·√[2(1 − α)]; α = fidedignidade interna), com mudança confiável quando '
 '|RCI| ≥ 1,96 (JACOBSON; TRUAX, 1991). Cada observação foi classificada nos seis perfis de humor por proximidade aos '
 'protótipos em escores padronizados. Adotou-se α = 0,05.')
...
P('A Tabela 2 apresenta a descritiva geral das seis subescalas e da PTH. As dimensões negativas de valência não fadiga '
 ...
 'reside a variabilidade útil do microciclo. As trajetórias diárias, com bandas de confiança de 95%, constam na Figura 4.')
def drow(k,lab):
...
P('Os histogramas (Figura 2) evidenciam distribuições assimétricas: o vigor aproxima-se da simetria e a fadiga tem leve '
 'assimetria à direita, ao passo que as subescalas negativas concentram-se no piso, com forte assimetria positiva. O '
 'teste de Shapiro-Wilk rejeitou a normalidade em todas as subescalas (p < 0,001; Tabela 3), justificando a adoção de '
 'estatística não paramétrica. A consistência das medidas repetidas ao longo da semana (ICC) foi moderada a boa para a '
 'maioria das subescalas, sendo mais baixa para a raiva e a confusão — as dimensões mais reativas dia a dia —; a média '
 'semanal, contudo, mostrou fidedignidade elevada (ICC(2,k) ≥ 0,81) em todas.')
...
P('As Tabelas 4 e 5 reúnem, para todas as subescalas, a resposta aguda pré→pós e a variação Dia 1 → Dia 7, com variação '
...
sig=[pr for pr in PA if pr['p']<0.05]
P('As correlações de Spearman entre as subescalas (nível entre atletas) revelam um agrupamento coeso de afeto negativo '
 '(Tabela 6): a depressão associa-se à raiva (ρ = %s; p = %s) e à confusão (ρ = %s; p = %s), a tensão à confusão '
...
table('Correlação de Spearman entre as subescalas do BRUMS (nível entre atletas, n = %d): coeficiente (ρ) e valor de p.'%sm['n'],
    ...
    [['%s × %s'%(pr['a'],pr['b']),c2('%+.2f'%pr['rho']),pstr(pr['p'])] for pr in PA],fs=9)
P('O contraste entre os dias extremos é o achado mais expressivo (Tabela 7). No dia de maior vigor (Dia 1, grupo '
...
P('No nível individual, a mudança confiável (RCI, Dia 1 → Dia 7) distribui-se de modo desigual entre as subescalas '
 '(Tabela 8; Figura 6). A fadiga é a dimensão em que a maioria dos atletas deteriora de forma confiável (%d de %d; %d%%), '
...
P('Na linha de base (Dia 1) predomina o perfil iceberg (%d%%), marca de prontidão; no último dia (Dia 7) o perfil de '
 'fadiga (“barbatana de tubarão”) torna-se o mais frequente (%d%%) e o iceberg cai para %d%% (Figura 7; Tabela 9). Essa '
