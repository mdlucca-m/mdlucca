...
for r in rows[4:]:
    if r[0] and _isid(r[0]):
        ...
        tc.append(dict(ID=str(r[0]).strip(), pos=r[2], reps_ini=f(r[5]), PV=f(r[10]), FCmax=f(r[11]), PVini=f(r[9])))
TC=pd.DataFrame(tc)
...
TC['dPV']=TC['PV']-TC['PVini']  # adaptação T-CAR1->T-CAR2
...
F.to_csv('tcar2_features.csv',index=False)
...
# ===== F) ADAPTAÇÃO (ΔPV) vs baseline e vs deterioração semanal =====
ADAPT={}
for ov in ['b_Vigor','b_Fadiga','b_FadFisica','b_ice_idx','d_FadFisica','d_Vigor','d_TMD']:
    ADAPT[ov]=dict(zip(('r','p','n'),sr('dPV',ov)))
RES['ADAPT']=ADAPT
RES['means']['PV']=float(F['PV'].mean()); RES['means']['dPV']=float(F['dPV'].mean())
json.dump(RES,open('tcar2_baseline.json','w'),indent=1)
...
print('\nF) ADAPTAÇÃO ΔPV (T-CAR2-T-CAR1) vs desfechos (ρ[p])  ΔPV médio=%.2f'%F['dPV'].mean())
for ov in ['b_Fadiga','b_ice_idx','d_FadFisica','d_Vigor']:
    d=ADAPT[ov]; print('  %-12s %+.2f[%.2f]'%(ov,d['r'],d['p']))
