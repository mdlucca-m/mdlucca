...
PS=jd("V2_psico"); TEJ=jd("V2_te")
ETV={t["variavel"]:t for t in TEJ["TE"]}; MMIV={m["variavel"]:m for m in TEJ["MMI"]}
INDV={i["variavel"]:i for i in TEJ["INDIVIDUAL"]}
...
for i,(sub,ps) in enumerate(A.METODO):
    ...
    if i==3:
        figura(f"{S}/F2fig.png",fig(),"As quatro unidades de análise que circulavam nas versões anteriores e o "
               "efeito de cada uma sobre a variação do perfil iceberg entre o primeiro e o sétimo dia.",w=16.0)
        caption(f"Quadro {quadro()} – Achados da auditoria de procedência e respectivas correções")
...
para(A.R1[0])
caption(f"Tabela {tab()} – Descrição robusta das sete variáveis nos 166 pares atleta-dia")
...
para(A.R1[1])
caption(f"Tabela {tab()} – Forma da distribuição, efeito de piso, posição normativa e confiabilidade")
...
para(A.R2[0])
figura(f"{S}/F4fig.png",fig(),"Trajetória das seis subescalas em escore T ao longo do microciclo e resultado do "
       "teste de tendência de Page.",w=16.0)
for p in A.R2[1:]: para(p)
caption(f"Tabela {tab()} – Comparação global entre os sete dias, tendência ordenada e contraste com a linha de base")
...
para(A.R3[0])
figura(f"{S}/F5fig.png",fig(),"Decomposição sinal–ruído das seis subescalas: série observada com erro-padrão, "
       "série suavizada e banda do piso de ruído.",w=16.0)
for p in A.R3[1:]: para(p)
caption(f"Tabela {tab()} – Piso de ruído, deslocamento, veredito, transições de choque e pontos de inflexão")
...
para(A.R5[0])
figura(f"{S}/F7fig.png",fig(),"Prevalência diária dos seis perfis de humor, com piso de ruído e transições de "
       "choque.",w=16.0)
for p in A.R5[1:]: para(p)
caption(f"Tabela {tab()} – Prevalência diária dos perfis e das faixas, veredito e teste de estabilidade")
rows=[]
...
para(A.R6[0])
figura(f"{S}/F9fig.png",fig(),"Custo do dia por tipo de estímulo e migração intradiária para a faixa de risco.",w=16.0)
for p in A.R6[1:]: para(p)
caption(f"Tabela {tab()} – Resposta aguda intradiária por tipo de estímulo e migração para a faixa de risco")
...
head("3.7 Confiabilidade do instrumento neste elenco", lvl=2)
para(A.RC[0])
caption(f"Tabela {tab()} – Consistência interna das seis subescalas do BRUMS nesta amostra")
mktable(["Subescala","α de Cronbach","IC 95% do α","ω de McDonald","IC 95% do ω","Adequada (α ≥ 0,70)"],
  [[c['subescala'], n_(c['alfa'],3),
    (f"{n_(c['alfa_ic'][0],3)}; {n_(c['alfa_ic'][1],3)}" if c['alfa_ic'][0] is not None else "—"),
    n_(c['omega'],3),
    (f"{n_(c['omega_ic'][0],3)}; {n_(c['omega_ic'][1],3)}" if c['omega_ic'][0] is not None else "—"),
    ("sim" if c['adequada'] else "não")] for c in PS['CONF']],
  widths=[3.0,2.4,2.6,2.4,2.6,2.6], fs=8.5)
src(nota=f"Sobre os {PS['n']} registros de {PS['atletas']} atletas. Intervalos por reamostragem agrupada por "
         "atleta, com 800 repetições, uma vez que os registros do mesmo atleta não são independentes. O ômega "
         "foi obtido de solução unifatorial por eixo principal iterado.")
para(A.RC[1])
caption(f"Tabela {tab()} – Comportamento item a item e escalonamento multitraço")
_mt={ (m['subescala'],m['item']): m for m in PS['MULTITRACO']['itens'] }
mktable(["Subescala","Item","r item-total corrigida","α sem o item","% no piso","Carga fatorial",
         "Escalonamento"],
  [[c['subescala'], it, n_(c['item_total'][j],2), n_(c['alfa_sem_item'][j],3), n_(c['pct_piso'][j],1),
    (n_(c['cargas'][j],2) if c['cargas'] else "—"),
    f"{_mt[(c['subescala'],it)]['vitorias']} de {_mt[(c['subescala'],it)]['de']}"]
   for c in PS['CONF'] for j,it in enumerate(c['itens'])],
  widths=[2.4,2.6,2.8,2.0,1.8,2.0,2.2], fs=7.5)
src(nota="O escalonamento multitraço conta as comparações em que o item se correlaciona mais com a própria "
         f"subescala, corrigida, do que com cada uma das outras cinco. No conjunto, "
         f"{PS['MULTITRACO']['sucessos']} de {PS['MULTITRACO']['total']} comparações são vencidas "
         f"({n_(PS['MULTITRACO']['pct'],1)}%).")
para(A.RC[2])

head("3.8 A semana como redistribuição: os perfis um a um e em conjunto", lvl=2)
para(A.RP[0])
figura(f"{S}/P1fig.png",fig(),"Prevalência diária de cada perfil de humor ao longo do microciclo, com "
       "erro-padrão binomial, série suavizada e dia de pico.",w=16.0)
para(A.RP[1])
figura(f"{S}/P2fig.png",fig(),"Composição do elenco entre os seis perfis, dia a dia, e as três faixas com o "
       "ponto de inversão entre a favorável e a de risco.",w=16.0)
para(A.RP[2])
...
caption(f"Tabela {tab()} – Erro típico, menor mudança relevante e mudança mínima detectável")
mktable(["Variável","Piso do grupo","Erro típico","IC 95% do ET","ET (%)","Menor mudança relevante",
         "Mudança mínima detectável","MMR ÷ ET","Aptidão"],
  [[L(v), n_(ETV[v]['piso_grupo']), n_(ETV[v]['et']),
    (f"{n_(ETV[v]['et_ic'][0])}; {n_(ETV[v]['et_ic'][1])}" if ETV[v]['et_ic'][0] is not None else "—"),
    n_(ETV[v]['et_pct'],1), n_(ETV[v]['mmr']), n_(ETV[v]['mmd95']), n_(ETV[v]['razao']),
    ETV[v]['aptidao']] for v in V7],
  widths=[1.7,1.7,1.4,2.1,1.2,2.2,2.3,1.3,1.7], fs=7.5)
src(nota="Erro típico = desvio das diferenças entre dias consecutivos do mesmo atleta ÷ √2. Menor mudança "
         "relevante = 0,2 × desvio-padrão entre atletas. Mudança mínima detectável = 1,96 × √2 × erro típico. "
         "A aptidão classifica a razão entre a menor mudança relevante e o erro típico: boa a partir de 1,0 e "
         "aceitável a partir de 0,5 (HOPKINS, 2000; LINDBERG et al., 2022).")
para(A.RL[2])
caption(f"Tabela {tab()} – Mudança mínima importante, ancorada na entrada na faixa de risco")
mktable(["Variável","AUC da variação","Ponto de corte","Sensibilidade","Especificidade","Corte ÷ ET",
         "Supera a MMR"],
  [[L(v), n_(MMIV[v]['auc'],3),
    (n_(MMIV[v]['corte'],1) if MMIV[v]['discrimina'] else "não discrimina"),
    (n_(MMIV[v]['sens'],2) if MMIV[v]['discrimina'] else "—"),
    (n_(MMIV[v]['espec'],2) if MMIV[v]['discrimina'] else "—"),
    (n_(MMIV[v]['corte_sobre_et']) if MMIV[v]['discrimina'] else "—"),
    ("sim" if MMIV[v]['supera_mmr'] else "não") if MMIV[v]['discrimina'] else "—"] for v in V7],
  widths=[2.2,2.6,2.4,2.4,2.6,1.9,2.2], fs=8)
src(nota=f"Sobre os {TEJ['n_casos_ancora']} pares atleta-dia que amanhecem fora da faixa de risco, dos quais "
         f"{TEJ['eventos_ancora']} entram nela até a noite. Ponto de corte pelo índice de Youden. Variáveis "
         "com área sob a curva inferior a 0,60 não discriminam e não recebem corte.")
para(A.RL[3])
caption(f"Tabela {tab()} – Resposta individual entre o primeiro e o sétimo dia, contra cada limiar")
mktable(["Variável","n","Δ médio","Acima do erro típico","Acima da MMR","Acima da MMD","Na direção do grupo"],
  [[L(v), str(INDV[v]['n']), n_(INDV[v]['delta_medio']), str(INDV[v]['acima_et']),
    str(INDV[v]['acima_mmr']), str(INDV[v]['acima_mmd']), str(INDV[v]['mesma_direcao'])] for v in V7],
  widths=[2.2,1.2,1.8,3.0,2.4,2.4,3.2], fs=8.5)
src(nota="Restrito aos atletas com medida no primeiro e no sétimo dia. «Na direção do grupo» conta quantos "
         "variam no mesmo sentido da média, independentemente da magnitude.")
para(A.RL[4])

head("3.10 Qualidade dos dados e reconferência dos resultados", lvl=2)
para(A.RQ[0]); para(A.RQ[1])
caption(f"Tabela {tab()} – Completude do instrumento e cobertura da grade atleta-dia")
...
para(A.RQ[2])
caption(f"Tabela {tab()} – Triagem de valores discrepantes: verificação de domínio e três critérios de dispersão")
...
para(A.RQ[3])
caption(f"Tabela {tab()} – Discrepantes intraindividuais: cada atleta contra a própria série")
