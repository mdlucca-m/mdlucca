...
// 3.13 Carga × humor e preditores de fadiga
const ch = JSON.parse(fs.readFileSync(path.join(HERE, '..', 'carga_humor', 'resultados_carga_humor.json'), 'utf8'));
const chP = ch.preditores_fadiga.preditores;
children.push(new Paragraph({ children: [new PageBreak()] }));
children.push(H('3.13. Carga interna × perfil de humor e preditores de estado de fadiga', HeadingLevel.HEADING_2));
children.push(P('Duas perguntas aplicadas fecham o arco carga→humor, mantendo o atleta como unidade (correlações no nível do atleta, n = ' + ch.acoplamento.tonico.n + ', com correção FDR) e validação por bootstrap agrupado por atleta. Primeira: a carga interna (PSE, FC de pico, TRIMP) liga-se a cada dimensão do humor? Estendendo o acoplamento a todo o perfil — as seis subescalas e os índices derivados —, o padrão é de desacoplamento: nenhum par carga × humor sobrevive à correção FDR, seja na leitura tônica (média do atleta nos dias de HIIT) ou aguda (Δ pós−pré). Restam apenas dois sinais fracos no p bruto (não após FDR): no agudo, PSE × Tensão (r = +0,41) e PSE × Confusão (r = +0,40) — a percepção de esforço acompanha ativação/confusão momentânea, não fadiga. A carga interna prescreve o estímulo; a resposta de humor carrega informação que a carga objetiva não captura, o que fundamenta a superioridade das medidas subjetivas no monitoramento da resposta ao treino (Saw, Main & Gastin, 2016; Haddad et al., 2017).'));
children.push(P('Segunda pergunta: entre as variáveis de humor, quais são simultaneamente sensíveis e confiáveis para sinalizar um estado de fadiga alta vs. baixa? Definindo o alvo como o tercil superior vs. inferior da subescala Fadiga (n = ' + ch.preditores_fadiga.n_alta + '/' + ch.preditores_fadiga.n_baixa + '; PTH excluído por conter aritmeticamente a Fadiga), cada preditor concorrente foi avaliado por AUC (Mann–Whitney; IC95% por bootstrap agrupado por atleta), sensibilidade e especificidade no ponto de Youden, e confiabilidade por ICC(2,1) entre medidas repetidas. A fadiga física é o marcador mais sensível (AUC ' + nf(chP[0].AUC) + '); seu ICC baixo (' + nf(chP.find(x=>x.var==='FadFis').ICC21) + ') é a assinatura esperada de um bom marcador de estado, que deve oscilar com a carga. A fadiga mental e a depressão combinam sensibilidade com estabilidade (ICC ' + nf(chP.find(x=>x.var==='FadMen').ICC21) + ' e ' + nf(chP.find(x=>x.var==='Depressão').ICC21) + '), sendo os marcadores confiáveis dia a dia; o vigor é sensível pelo eixo energético; e a tensão, apesar de a mais estável, é praticamente cega à fadiga. Por um caminho independente — classificação/ROC — reencontra-se o eixo energia–fadiga dos modelos mistos, da análise fatorial e da invariância.'));
children.push(table(
  ['Preditor', 'AUC', 'IC95%', 'Sensib.', 'Especif.', 'ICC(2,1)'],
  chP.map(r => [r.label, nf(r.AUC), '[' + nf(r.IC95[0]) + '; ' + nf(r.IC95[1]) + ']', nf(r.sensibilidade), nf(r.especificidade), r.ICC21 == null ? '—' : nf(r.ICC21)]),
  [3160, 1240, 1960, 1000, 1000, 1000], ['l', 'r', 'r', 'r', 'r', 'r']));
children.push(caption('Tabela 11. Preditores de estado de fadiga alta vs. baixa: sensibilidade (AUC, Youden) e confiabilidade (ICC 2,1). A fadiga física lidera a sensibilidade; fadiga mental e depressão reúnem sensibilidade e estabilidade.'));
{
  const buf = fs.readFileSync(path.join(HERE, '..', 'carga_humor', 'carga_humor_fig.png'));
  const wPx = 660, hPx = Math.round(wPx * 897 / 2016);
  children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 120, after: 40 },
    children: [new ImageRun({ type: 'png', data: buf, transformation: { width: wPx, height: hPx } })] }));
  children.push(caption('Figura 2. Carga interna × perfil de humor (acoplamento tônico e agudo, com FDR) e preditores de estado de fadiga no plano sensibilidade (AUC) × confiabilidade (ICC 2,1).'));
}

// 3.14 Discussão integrada
children.push(H('3.14. Discussão integrada: dos objetivos aos achados', HeadingLevel.HEADING_2));
children.push(P([bold('Interpretação estatística. '), run('A inferência apoia-se em modelos mistos com o atleta como efeito aleatório, que separam variância de traço, dia e estado — condição para falar de resposta ao treino sem pseudorreplicação. Sobre essa base, controle de multiplicidade por FDR, tamanhos de efeito intra-sujeito (dz) e bootstrap agrupado por atleta dão robustez; e a convergência entre famílias que raramente erram juntas (t e Wilcoxon/Friedman, frequentista e bayesiana, univariada e multivariada) indica que o achado é do fenômeno, não do método.')]));
children.push(P([bold('Interpretação fisiológica. '), run('As sessões foram quase-máximas (FC de pico 181–184 bpm; %HRR 0,87–0,91; PSE final 9,3–9,6). O custo alto e homogêneo traduz-se em fadiga física (dz ≈ 1,0), que acumula e é a única dimensão amplificada pelo HIIT no agudo. O desacoplamento carga↔humor mostra que, num regime de teto, a variação relevante do humor depende de fatores individuais de tolerância e recuperação, não do custo cardiovascular da sessão.')]));
children.push(P([bold('Interpretação psicológica. '), run('A mudança pré→pós é um deslocamento de estado sobre o eixo energia–fadiga (vigor achata, fadiga sobe; negativas quase paradas) — erosão do perfil "iceberg". A invariância de medida (métrica sustentada; escalar aproximada; estrita no limite, com a quebra localizada no item "Sonolento") confirma que o HIIT muda o estado medido, não o significado do instrumento; o deslocamento latente recai sobre fadiga (+0,56) e vigor (−0,26). O padrão é coerente com a resposta afetiva a exercício intervalado intenso (Marques et al., 2020; Patten et al., 2022).')]));
children.push(P([bold('Pontos fortes, validade e impactos. '), run('Disciplina de atleta-como-unidade, triangulação por métodos independentes, caracterização psicométrica completa (confiabilidade, AFC, AFE, TRI, invariância configural→estrita/parcial) e reprodutibilidade por código. Validade convergente (BRUMS ↔ autorrelatos externos), de construto (invariância) e diagnóstica (AUC). Impacto aplicado: um protocolo de monitoramento parcimonioso — fadiga física (sensível) e fadiga mental/depressão (estáveis) — para individualizar carga e recuperação; com a ressalva honesta de que a FC/TRIMP não substituem o autorrelato e de que decisões pela média do grupo podem prejudicar subgrupos.')]));
