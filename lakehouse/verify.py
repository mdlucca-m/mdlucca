...
GOLD = ["athlete_day", "daily_group", "acute_prepos", "risk_features",
        ...
        "an_two_omnibus", "an_two_dunnett", "an_two_daytype", "an_two_mixed", "an_two_payload", "risk_features"]
...
def main():
    ...
    import ingest, dbt_run, analytics, analytics_physical, analytics_panel, analytics_tri, analytics_twopaths
    ingest.run(); dbt_run.run(); analytics.run(); analytics_physical.run(); analytics_panel.run(); analytics_tri.run(); analytics_twopaths.run()
    s2 = sig_all()
