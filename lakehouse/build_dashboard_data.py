...
def gen_DOSE():
    """DOSE ← gold.an_dose_payload: modelo dose-resposta (carga contínua)."""
    return "const DOSE=" + _payload("an_dose_payload")

def gen_STR():
    """STR ← gold.an_str_payload: análise estratificada HIIT × sem-HIIT + PV log + logística."""
    return "const STR=" + _payload("an_str_payload")

def gen_RSM():
    """RSM ← gold.an_rsm_payload: superfície de resposta de 2ª ordem (carga aguda × cumulativa)."""
    return "const RSM=" + _payload("an_rsm_payload")

def gen_NET():
    """NET ← gold.an_net_payload: rede GGM + painel cruzado defasado (CLPM)."""
    return "const NET=" + _payload("an_net_payload")
...
def run():
    ...
    gens = {"DIM": gen_DIM(), "D17": gen_D17(), "FRIED": gen_FRIED(),
            ...
            "CROSS": f"const CROSS={gen_CROSS()}", "CURVE_CROSS": f"const CURVE_CROSS={gen_CROSS()}", "MV": gen_MV(), "SENSV": gen_SENSV(), "HVS": gen_HVS(), "SENSA": gen_SENSA(), "IOTPRED": gen_IOTPRED(), "DERIV": gen_DERIV(), "TRI": gen_TRI(), "TWO": gen_TWO(), "NORMSTD": gen_NORM(), "TRICONF": gen_TRICONF(), "FAC": gen_FAC(), "DYN": gen_DYN(), "LOAD": gen_LOAD(), "DOSE": gen_DOSE(), "STR": gen_STR(), "RSM": gen_RSM(), "NET": gen_NET()}
