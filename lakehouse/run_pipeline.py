...
"""
import time
import ingest, dbt_run, analytics, analytics_physical, ml_risk, build_dashboard_data, export_dashboard, lh

def main():
    t0 = time.time()
    print("== 1/6 BRONZE (ingestão bruta) ==");   ingest.run()
    print("== 2/6 SILVER+GOLD via dbt-duckdb (SQL + testes) =="); dbt_run.run()
    print("== 3/6 GOLD·ANÁLISES (todas as análises) =="); analytics.run(); analytics_physical.run()
