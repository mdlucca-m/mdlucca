...
"""
import time
import ingest, dbt_run, analytics, analytics_physical, analytics_panel, analytics_tri, analytics_twopaths, analytics_norm, analytics_tri_confirm, analytics_factorial, analytics_dynamics, analytics_load, analytics_doseresp, analytics_strata, analytics_rsm, analytics_network, analytics_multiplicity, analytics_game, ml_risk, build_dashboard_data, export_dashboard, lh

def main():
    t0 = time.time()
    print("== 1/6 BRONZE (ingestão bruta) ==");   ingest.run()
    print("== 2/6 SILVER+GOLD via dbt-duckdb (SQL + testes) =="); dbt_run.run()
    print("== 3/6 GOLD·ANÁLISES (todas as análises) =="); analytics.run(); analytics_physical.run(); analytics_panel.run(); analytics_tri.run(); analytics_twopaths.run(); analytics_norm.run(); analytics_tri_confirm.run(); analytics_factorial.run(); analytics_dynamics.run(); analytics_load.run(); analytics_doseresp.run(); analytics_strata.run(); analytics_rsm.run(); analytics_network.run(); analytics_multiplicity.run(); analytics_game.run()
